...
func (l Log) Run() {
	...
	for {
		...
		case val := <-bg.In:
			...
			switch log.Type {
			case "container":
				fmt.Printf("%-35v | Source:%-20s | Type:%-10s | Action:%-10s | Host:%-20s | %-50s  >> Log: %v\n", ts, log.Source, log.Type, log.Action, log.Host, log.GetCntInfo(), log.Msg)
			case "image":
				fmt.Printf("%-35v | Source:%-20s | Type:%-10s | Action:%-10s | Host:%-20s >> Log: %v\n", ts, log.Source, log.Type, log.Action, log.Host, log.Image)
			}
		}
	}
}
